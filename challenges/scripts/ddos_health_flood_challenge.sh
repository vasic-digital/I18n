...
# I18n per CONST-035 + CONST-050(B). Submodule cascade per
# CONST-051(A). Targets $I18N_HEALTH_URL.
...
echo "=== I18n DDoS Health-Flood Challenge ==="
echo "  url=$HEALTH_URL total=$TOTAL_REQS conc=$CONCURRENCY pass≥${MIN_PASS_PCT}%"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/5] SKIP: I18N_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== I18n DDoS Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== I18n DDoS Challenge: PASSED ==="
